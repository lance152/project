...
def wtire_to_file(data):
    with open('yelp-vocab.txt',encoding='utf-8',mode='w') as f:
        index = 1
        for t in data:
            print(t[1])
...
def run():
    hist = process_file('yelp-train.txt')
    data = most_common(hist,10)

    wtire_to_file(data)
    list=getlist()
    trainmat = getmatric(list, 'yelp-train.txt', 'yelp-trainbyline.txt','yelp-trainclasss.txt','yelp-trainmatbag.txt')
    testmat=getmatric(list,'yelp-test.txt','yelp-testbyline.txt','yelp-testclasss.txt','yelp-testmatbag.txt')
    validmat=getmatric(list,'yelp-valid.txt','yelp-validbyline.txt','yelp-validclasss.txt','yelp-validmatbag.txt')
    np.savetxt('yelp-trainmat.txt', trainmat, fmt="%d", delimiter=",")
    np.savetxt('yelp-testmat.txt', testmat, fmt="%d", delimiter=",")
    np.savetxt('yelp-validmat.txt', validmat,fmt="%d",delimiter=",")
